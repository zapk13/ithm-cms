<div x-data="admissionForm()" class="max-w-4xl mx-auto">
    <div class="mb-8 flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Submit New Admission</h1>
            <p class="text-slate-500 mt-1">Submit an admission application on behalf of a student</p>
        </div>
        <a href="<?= BASE_URL ?>/admin/admissions" class="px-4 py-2 border border-slate-300 rounded-xl text-slate-700 hover:bg-slate-100">
    ...
    <form method="POST" action="<?= BASE_URL ?>/admin/admissions/submit" enctype="multipart/form-data" @submit="submitting = true">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <p x-show="!loading && formData.campus_id && courses.length === 0" class="text-amber-600 text-sm mt-1">
                            <i class="fas fa-exclamation-triangle mr-1"></i>No courses found. Please add courses in Course Management.
                        </p>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" x-model="formData.email" required
                               class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500">
                        <p class="text-slate-500 text-xs mt-1">Student will use this email to login</p>
                    </div>
                    <div>
            ...
            <div x-show="currentStep === 4" class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Passport Photo</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">CNIC Front</label>
                        <input type="file" name="cnic_front" accept="image/*,.pdf"
                               class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100">
                    </div>
                    <div>
...
function admissionForm() {
    return {
        ...
        async loadCourses() {
            ...
            try {
                const url = `<?= BASE_URL ?>/admin/api/campuses/${this.formData.campus_id}/courses`;
        ...
        // Get available shifts for selected course
        getAvailableShifts() {
            if (!this.formData.course_id || this.courses.length === 0) {
                return ['morning', 'evening'];
            }
            const course = this.courses.find(c => c.id == this.formData.course_id);
            if (course && course.shifts) {
                return course.shifts.split(',');
            }
            return ['morning', 'evening'];
        }
    }
